...
//
// Jean-Louis Naudin (JLN) updates: last update 2012-08-15
//
// DOLIST:
//
//-------------------------------------------------------------------------------------------------------------------------
...
static void update_alt()
{
		...
		int32_t fake_relative_alt = g_gps->altitude - gps_base_alt;
		baro_alt       = fake_relative_alt;
	#else        
        if(g_gps->fix) { 
           // use barometer altitude ( altitude_mix = 1 ) or if 0 use gps_altitude only for the climb rate calculation
          baro_alt = (1 - g.altitude_mix) * (g_gps->altitude - gps_base_alt);	// alt_MSL centimeters (meters * 100)
          baro_alt += g.altitude_mix * read_barometer();  
        } else {
